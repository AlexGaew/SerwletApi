...
import ResumeWebAPP.model.LanguageLevel;
import ResumeWebAPP.model.LanguageType;
...
public class TestDataGenerator {
    ...
    private static final String PHOTO_PATH = "D:\\WorkSpaseUltimate\\SerwletApi\\external\\test-data\\photos";
    private static final String CERTIFICATE_PATH = "D:\\WorkSpaseUltimate\\SerwletApi\\external\\test-data\\certificates";

    private static final String MEDIA_DIR = "D:\\WorkSpaseUltimate\\SerwletApi\\web\\media";
    private static final String COUTRY = "Russia";

    private static final String[] CITIES = {"Moskov", "St'Peterburg", "Krasnodar", "Voroneg"};
    private static final String[] FOREGIN_LANGUAGES = {"English", "Spanish", "German", "Italian"};
    ...
    static {
        ...
        List<String> stringList = new ArrayList<>(sentences.length);
        ...
        SENTENCES = Collections.unmodifiableList(stringList);
    }
    ...
    private static void clearMedia() throws Exception {
    ...
    private static List<Profile> loadProfiles() {
        ...
        Collections.sort(list, Comparator.comparing((Profile o) -> o.firstName).thenComparing(o -> o.lastName));
        return list;
    }
    ...
    private static void createProfile(Connection c, Profile profile, ProfileConfig profileConfig,
                                      List<Certificate> certiicates) throws SQLException, IOException, IllegalAccessException {
        insertProfileData(c, profile, profileConfig);
        if (profileConfig.certificates > 0) {
            insertCertificates(c, profileConfig.certificates, certiicates);
        }
        insertEducation(c);
        insertHobbies(c);
        insertLanguages(c);
    ...
    private static void insertPractics(Connection c, ProfileConfig profileConfig) throws SQLException {
        ...
        for (Course course : profileConfig.courses) {
            ...
            if (currentCourse) {
                ps.setDate(4, (java.sql.Date) addField(new Date(System.currentTimeMillis()), Calendar.MONTH, -1, false));
                ps.setNull(5, Types.DATE);
            } else {
                ps.setDate(4, (java.sql.Date) addField(finish, Calendar.MONTH, -1, false));
                ps.setDate(5, (java.sql.Date) finish);
    ...
    private static void insertLanguages(Connection c) throws SQLException, IllegalAccessException {
        ...
        for (String language : languages) {
            ...
            if (langType != LanguageType.ALL) {
                ...
                ps.setString(3, langLevel.getDbValue());
                ps.setString(4, langType.getReserveType().getDbValue());
                ps.addBatch();
            }
        }
        ps.executeBatch();
